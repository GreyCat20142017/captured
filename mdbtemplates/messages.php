<main class="container">
    ...
    <section class="row p-4">
                                ...
                                <div class="d-flex pt-2">
                                    <div class="position-relative mr-2 pl-3">
                                        <img class="rounded-circle" width="60" height="60"
                                             src="<?= get_avatar(get_pure_data($correspondent,
                                                 'correspondent_avatar')); ?>"
                                             alt="Аватар пользователя">
                                        <?php if (!empty(get_pure_data($correspondent, 'unread_count'))): ?>
                                            <i class="position-absolute text-white mdb-color rounded-circle py-1 px-2"
                                            ...
                                            <p class="messages__preview-text">
                                                ...
                                                <small>
                                                    <time class="messages__preview-time" datetime="2019-05-01T14:40"
                                                        ...
                                                        <?= date('H:i',
                                                            strtotime(get_pure_data($correspondent,
                                                                'creation_date'))); ?>
                                                    </time>
                                                </small>
                                            </p>
                ...
                <?php else: ?>
                    ...
                    <div class="comments mx-auto">
                        <form class="comments__form form  d-flex flex-column w-100"
                            ...
                            <button class="btn btn-mdb-color comments__submit" type="submit" name="add_message">
                                Отправить
                            </button>
                        </form>
                    </div>
        ...
        <?php if (empty($correspondents) && empty($active_user)): ?>
            <p>На текущий момент у Вас нет активных переписок. </p>
            <p>Для начала обмена сообщениями с кем-либо нужно зайти в профиль интересующего Вас автора
                и воспользоваться кнопкой "Сообщение".</p>
        <?php endif; ?>
    </section>
</main>

<main class="page__main page__main--profile container">
    <h1 class="visually-hidden">Профиль</h1>

    <div class="profile profile--<?= $active_tab; ?>">
        <div class="profile__user-wrapper">
            <div class="profile__user user container d-flex py-4 align-items-stretch">
                ...
                <div class="ml-auto font-weight-bold">
                    <p class="d-flex flex-column align-items-center">
                        <span class="indigo-text h4-responsive"><?= isnull(get_pure_data($user, 'posts_count'), 0); ?></span>
            ...
            <div class="btn-group" role="group" aria-label="Показать:">
                <a class="btn btn-indigo btn-rounded <?= get_mdb_active($active_tab,
                   ...
                   href="<?= rebuild_query_string($active_script, $active_query, 'tab', 1); ?>">
                    <i class="fas fa-star fa-sm pr-2" aria-hidden="true"></i>
                    Посты</a>
                <a class="btn btn-indigo btn-rounded <?= get_mdb_active($active_tab,
                    LIKES); ?>"
                   href="<?= rebuild_query_string($active_script, $active_query, 'tab', 2); ?>">
                    <i class="fas fa-heart fa-sm pr-2"
                       aria-hidden="true"></i>Лайки</a>
                <a class="btn btn-indigo btn-rounded <?= get_mdb_active($active_tab,
                    SUBSCRIPTIONS); ?>"
                   href="<?= rebuild_query_string($active_script, $active_query, 'tab', 3); ?>">
                    <i class="fas fa-user fa-sm pr-2" aria-hidden="true"></i>Подписки</a>
            </div>
        </div>
